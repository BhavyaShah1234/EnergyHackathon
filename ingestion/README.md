...
| **EIA NG Pipelines** (Interstate/Intrastate routes, WECC-SW + ERCOT) | `pipelines_infra` | Weekly | Collide sub-B | [U.S. Energy Atlas](https://atlas.eia.gov/) · [FeatureServer](https://services2.arcgis.com/FiaPA4ga0iQKduv3/arcgis/rest/services/Natural_Gas_Interstate_and_Intrastate_Pipelines_1/FeatureServer/0) |
...
> Teammate quickstart: [`docs/pipelines_infra.md`](docs/pipelines_infra.md) — pull steps, field dictionary, and known gaps.
>
...
| `operator` | `str` | nullable | Pipeline operator name |
| `status` | `str` | nullable | Operational status |
| `start_lon` / `start_lat` | `float` | North America envelope | First vertex of the polyline |
| `end_lon` / `end_lat` | `float` | North America envelope | Last vertex of the polyline |
| `midpoint_lon` / `midpoint_lat` | `float` | North America envelope | Unweighted centroid of all vertices |
| `length_km` | `float` | [0, 5000] | Great-circle length along the polyline |
| `num_vertices` | `int` | ≥ 2 | Polyline vertex count |
| `geometry_wkt` | `str` | not null | OGC `LINESTRING (lon lat, ...)` |

**Natural key:** `(pipeline_id)` · **Freshness SLA:** 168h · **Scope:** WECC-SW + ERCOT bbox (`-125..-93`, `25..42`)

> Known gap: the EIA public layer does not expose `install_year` or `pipe_material`. Those must come from PHMSA annual reports once bulk data is available.
...
pipeline/
  ...
  sources/
    ...
    pipelines_infra.py   EIA interstate/intrastate gas pipeline routes
orchestrator/
  run_once.py            One-shot backfill/catch-up
  run_live.py            Continuous (APScheduler) polling runner
docs/
  pipelines_infra.md     Pull + interpretation notes for the pipeline routes source
config/
